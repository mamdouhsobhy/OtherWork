...
class AdapterGasoline(private val itemSelected:(GasolineModel)->Unit) :
    ...
    override fun onCreateViewHolder(
        ...
        val binding =
            LayoutGasolineItemBinding.inflate(parent.layoutInflater, parent, false)
    ...
    private inner class ViewHolder(private val binding: LayoutGasolineItemBinding) :
        RecyclerView.ViewHolder(binding.root) {
        @SuppressLint("SetTextI18n", "NotifyDataSetChanged")
        fun bind(item: GasolineModel) = with(binding) {

            tvGasolineValue.text = item.title
            val colorInt = Color.parseColor(item.color)
            layoutGasolineItem.backgroundTintList = ColorStateList.valueOf(colorInt)

            binding.layoutGasolineItem.setOnClickListener {
            ...
            if(item.selected){
                parentGasoline.setBackgroundResource(R.drawable.drawable_border_select)
            }else{
                parentGasoline.background = null
            }
